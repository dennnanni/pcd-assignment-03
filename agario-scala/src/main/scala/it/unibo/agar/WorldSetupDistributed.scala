...
object DistributedMain:
  def main(args: Array[String]): Unit =
    ...
    val playerSystem = startup("agario", 25256) {
      Behaviors.setup[Nothing] { ctx =>
        val sharding = ClusterSharding(ctx.system)
        ctx.log.info(s"Starting sharding in system ${ctx.system}")
        sharding.init(Entity(ZoneActor.TypeKey)(_ => Behaviors.empty))
        Behaviors.empty
      }
    }

    val playerRef = playerSystem.systemActorOf(PlayerActorDis("player1"), "player1")

    Thread.sleep(10000)

    playerRef ! PlayerActorDis.Spawn(350) // Player in zone4
    playerRef ! PlayerActorDis.Spawn(150) // Player in zone2
    playerRef ! PlayerActorDis.Spawn(250) // Player in zone3
